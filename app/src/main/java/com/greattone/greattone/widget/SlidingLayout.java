...
//		}
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (!isLeftLayoutVisible && moveDistanceX >= touchSlop
				...
				rightLayout.setLayoutParams(rightLayoutParams);
				thirdLayout.setLayoutParams(rightLayoutParams);
			}
			if (isLeftLayoutVisible && -moveDistanceX >= touchSlop) {
				isSliding = true;
 ...
 */
	...
	private void initThirdView() {
		thirdLayout=new RelativeLayout(getContext());
		LayoutParams layoutParams = new LayoutParams (screenWidth/3,LayoutParams.MATCH_PARENT);
		layoutParams.addRule(RelativeLayout.ALIGN_PARENT_RIGHT); // 设置与父控件的相对属性
		thirdLayout.setLayoutParams(layoutParams);
		thirdLayout.setBackgroundColor(getResources().getColor(android.R.color.transparent));
		thirdLayout.setOnTouchListener(this);
		addView(thirdLayout);
	}
