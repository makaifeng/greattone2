...
public class NewsContentItemGridAdapter extends BaseAdapter {
	...
	public NewsContentItemGridAdapter(Context context, List<Blog> newsList,
			int pager_style) {
...
//			 } else if (pager_style == PAGER_STYLE_MORE_PICTURE) {//多图样式
	...
	private void initVideoView(int position, View convertView, ViewHolder holder) {
		...
		ImageLoaderUtil.getInstance().setImagebyurl(
				newsList.get(position).getTitlepic(), holder.icon);
	...
	private void initOnePictureView(int position, View convertView,
		...
		ImageLoaderUtil.getInstance().setImagebyurl(
				newsList.get(position).getTitlepic(), holder.icon);
		holder.title.setText(newsList.get(position).getTitle());
		holder.content.setText(newsList.get(position).getSmalltext());
	}
