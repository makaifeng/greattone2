...
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = DisplayUtil.getDiskCacheDir(MyApplication.getInstance(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, DisplayUtil.getAppVersion(MyApplication.getInstance()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void displayVideo(String path, ImageView imageView, int width, int height) {
    	...
    	if (bitmap == null) {
    		//从文件中加载
    		BitmapLoadTask bitmapLoadTask = new BitmapLoadTask(path, imageView,1);
    ...
    public void display(String path, ImageView imageView, int width, int height) {
    	...
    	if (bitmap == null) {
    		//从文件中加载
    		BitmapLoadTask bitmapLoadTask = new BitmapLoadTask(path, imageView,0);
    		addTask(bitmapLoadTask);
    	} else {
    		ImageHolder imageHolder = new ImageHolder();
    		imageHolder.bitmap = bitmap;
    ...
    public class BitmapLoadTask extends AsyncTask<Integer, Object, Bitmap> {
        ...
        public BitmapLoadTask(String path, ImageView container, int type) {
    ...
    class DiskCacheThread extends Thread {
        ...
        public void run() {
            try {
                String key = DisplayUtil.hashKeyForDisk(path);
