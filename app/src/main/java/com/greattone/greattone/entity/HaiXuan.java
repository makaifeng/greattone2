package com.greattone.greattone.entity;

public class HaiXuan {
String id;
String classid;
String ttid;
String onclick;
String plnum;
String totaldown;
String newspath;
String filename;
String userid;
String username;
String firsttitle;
String isgood;
String ispic;
String istop;
String isqf;
String ismember;
String isurl;
String truetime;
String lastdotime;
String havehtml;
String groupid;
String userfen;
String titlefont;
String titleurl;
String stb;
String fstb;
String restb;
String keyboard;
String title;
String newstime;
String titlepic;
String intro;
String price;
String pmaxnum;
String newstext;
String psalenum;
String huodong_1;
String huodong_2;
String productno;
String pbrand;
String unit;
String weight;
String tprice;
String buyfen;
String productpic;
String dizhi;
String newstext_1;
String ke_type;
String ke_hour;
String ke_week;
String ke_province;
String ke_city;
String ke_area;
String ke_teacher;
String faname;
String huo_type;
String hai_name;
String hai_phone;
String hai_mailbox;
String hai_division;
String hai_grouping;
String hai_address;
String hai_age;
String hai_mend;
String hai_piano;
String hai_photo;
String hai_petition;
String hai_video;
String hai_cost;
String keyid;
String dokey;
String newstempid;
String closepl;
String infotags;
String isfava;
String resendnum;
String smalltext;
String notimg;
String havefava;
String allphoto;
String morepic;
String morepicTotle;
String picsay;
String onlinepath;
String flashurl;
String picurl;
String tbname;
String classname;
String downpath;
String bitype;
String hai_baotype;//报名上传类型
int ishistory;//海选是否过期  0没过期，1过期
int isstart;//海选是否开始  0没开始，1开始
	int hai_bao_group;//报名选用界面类型

	public int getHai_bao_group() {
		return hai_bao_group;
	}

	public void setHai_bao_group(int hai_bao_group) {
		this.hai_bao_group = hai_bao_group;
	}

	public String getHai_baotype() {
	return hai_baotype;
}
public void setHai_baotype(String hai_baotype) {
	this.hai_baotype = hai_baotype;
}
public int getIshistory() {
	return ishistory;
}
public void setIshistory(int ishistory) {
	this.ishistory = ishistory;
}
public int getIsstart() {
	return isstart;
}
public void setIsstart(int isstart) {
	this.isstart = isstart;
}
public String getBitype() {
	return bitype;
}
public void setBitype(String bitype) {
	this.bitype = bitype;
}
public String getId() {
	return id;
}
public void setId(String id) {
	this.id = id;
}
public String getClassid() {
	return classid;
}
public void setClassid(String classid) {
	this.classid = classid;
}
public String getTtid() {
	return ttid;
}
public void setTtid(String ttid) {
	this.ttid = ttid;
}
public String getOnclick() {
	return onclick;
}
public void setOnclick(String onclick) {
	this.onclick = onclick;
}
public String getPlnum() {
	return plnum;
}
public void setPlnum(String plnum) {
	this.plnum = plnum;
}
public String getTotaldown() {
	return totaldown;
}
public void setTotaldown(String totaldown) {
	this.totaldown = totaldown;
}
public String getNewspath() {
	return newspath;
}
public void setNewspath(String newspath) {
	this.newspath = newspath;
}
public String getFilename() {
	return filename;
}
public void setFilename(String filename) {
	this.filename = filename;
}
public String getUserid() {
	return userid;
}
public void setUserid(String userid) {
	this.userid = userid;
}
public String getUsername() {
	return username;
}
public void setUsername(String username) {
	this.username = username;
}
public String getFirsttitle() {
	return firsttitle;
}
public void setFirsttitle(String firsttitle) {
	this.firsttitle = firsttitle;
}
public String getIsgood() {
	return isgood;
}
public void setIsgood(String isgood) {
	this.isgood = isgood;
}
public String getIspic() {
	return ispic;
}
public void setIspic(String ispic) {
	this.ispic = ispic;
}
public String getIstop() {
	return istop;
}
public void setIstop(String istop) {
	this.istop = istop;
}
public String getIsqf() {
	return isqf;
}
public void setIsqf(String isqf) {
	this.isqf = isqf;
}
public String getIsmember() {
	return ismember;
}
public void setIsmember(String ismember) {
	this.ismember = ismember;
}
public String getIsurl() {
	return isurl;
}
public void setIsurl(String isurl) {
	this.isurl = isurl;
}
public String getTruetime() {
	return truetime;
}
public void setTruetime(String truetime) {
	this.truetime = truetime;
}
public String getLastdotime() {
	return lastdotime;
}
public void setLastdotime(String lastdotime) {
	this.lastdotime = lastdotime;
}
public String getHavehtml() {
	return havehtml;
}
public void setHavehtml(String havehtml) {
	this.havehtml = havehtml;
}
public String getGroupid() {
	return groupid;
}
public void setGroupid(String groupid) {
	this.groupid = groupid;
}
public String getUserfen() {
	return userfen;
}
public void setUserfen(String userfen) {
	this.userfen = userfen;
}
public String getTitlefont() {
	return titlefont;
}
public void setTitlefont(String titlefont) {
	this.titlefont = titlefont;
}
public String getTitleurl() {
	return titleurl;
}
public void setTitleurl(String titleurl) {
	this.titleurl = titleurl;
}
public String getStb() {
	return stb;
}
public void setStb(String stb) {
	this.stb = stb;
}
public String getFstb() {
	return fstb;
}
public void setFstb(String fstb) {
	this.fstb = fstb;
}
public String getRestb() {
	return restb;
}
public void setRestb(String restb) {
	this.restb = restb;
}
public String getKeyboard() {
	return keyboard;
}
public void setKeyboard(String keyboard) {
	this.keyboard = keyboard;
}
public String getTitle() {
	return title;
}
public void setTitle(String title) {
	this.title = title;
}
public String getNewstime() {
	return newstime;
}
public void setNewstime(String newstime) {
	this.newstime = newstime;
}
public String getTitlepic() {
	return titlepic;
}
public void setTitlepic(String titlepic) {
	this.titlepic = titlepic;
}
public String getIntro() {
	return intro;
}
public void setIntro(String intro) {
	this.intro = intro;
}
public String getPrice() {
	return price;
}
public void setPrice(String price) {
	this.price = price;
}
public String getPmaxnum() {
	return pmaxnum;
}
public void setPmaxnum(String pmaxnum) {
	this.pmaxnum = pmaxnum;
}
public String getNewstext() {
	return newstext;
}
public void setNewstext(String newstext) {
	this.newstext = newstext;
}
public String getPsalenum() {
	return psalenum;
}
public void setPsalenum(String psalenum) {
	this.psalenum = psalenum;
}
public String getHuodong_1() {
	return huodong_1;
}
public void setHuodong_1(String huodong_1) {
	this.huodong_1 = huodong_1;
}
public String getHuodong_2() {
	return huodong_2;
}
public void setHuodong_2(String huodong_2) {
	this.huodong_2 = huodong_2;
}
public String getProductno() {
	return productno;
}
public void setProductno(String productno) {
	this.productno = productno;
}
public String getPbrand() {
	return pbrand;
}
public void setPbrand(String pbrand) {
	this.pbrand = pbrand;
}
public String getUnit() {
	return unit;
}
public void setUnit(String unit) {
	this.unit = unit;
}
public String getWeight() {
	return weight;
}
public void setWeight(String weight) {
	this.weight = weight;
}
public String getTprice() {
	return tprice;
}
public void setTprice(String tprice) {
	this.tprice = tprice;
}
public String getBuyfen() {
	return buyfen;
}
public void setBuyfen(String buyfen) {
	this.buyfen = buyfen;
}
public String getProductpic() {
	return productpic;
}
public void setProductpic(String productpic) {
	this.productpic = productpic;
}
public String getDizhi() {
	return dizhi;
}
public void setDizhi(String dizhi) {
	this.dizhi = dizhi;
}
public String getNewstext_1() {
	return newstext_1;
}
public void setNewstext_1(String newstext_1) {
	this.newstext_1 = newstext_1;
}
public String getKe_type() {
	return ke_type;
}
public void setKe_type(String ke_type) {
	this.ke_type = ke_type;
}
public String getKe_hour() {
	return ke_hour;
}
public void setKe_hour(String ke_hour) {
	this.ke_hour = ke_hour;
}
public String getKe_week() {
	return ke_week;
}
public void setKe_week(String ke_week) {
	this.ke_week = ke_week;
}
public String getKe_province() {
	return ke_province;
}
public void setKe_province(String ke_province) {
	this.ke_province = ke_province;
}
public String getKe_city() {
	return ke_city;
}
public void setKe_city(String ke_city) {
	this.ke_city = ke_city;
}
public String getKe_area() {
	return ke_area;
}
public void setKe_area(String ke_area) {
	this.ke_area = ke_area;
}
public String getKe_teacher() {
	return ke_teacher;
}
public void setKe_teacher(String ke_teacher) {
	this.ke_teacher = ke_teacher;
}
public String getFaname() {
	return faname;
}
public void setFaname(String faname) {
	this.faname = faname;
}
public String getHuo_type() {
	return huo_type;
}
public void setHuo_type(String huo_type) {
	this.huo_type = huo_type;
}
public String getHai_name() {
	return hai_name;
}
public void setHai_name(String hai_name) {
	this.hai_name = hai_name;
}
public String getHai_phone() {
	return hai_phone;
}
public void setHai_phone(String hai_phone) {
	this.hai_phone = hai_phone;
}
public String getHai_mailbox() {
	return hai_mailbox;
}
public void setHai_mailbox(String hai_mailbox) {
	this.hai_mailbox = hai_mailbox;
}
public String getHai_division() {
	return hai_division;
}
public void setHai_division(String hai_division) {
	this.hai_division = hai_division;
}
public String getHai_grouping() {
	return hai_grouping;
}
public void setHai_grouping(String hai_grouping) {
	this.hai_grouping = hai_grouping;
}
public String getHai_address() {
	return hai_address;
}
public void setHai_address(String hai_address) {
	this.hai_address = hai_address;
}
public String getHai_age() {
	return hai_age;
}
public void setHai_age(String hai_age) {
	this.hai_age = hai_age;
}
public String getHai_mend() {
	return hai_mend;
}
public void setHai_mend(String hai_mend) {
	this.hai_mend = hai_mend;
}
public String getHai_piano() {
	return hai_piano;
}
public void setHai_piano(String hai_piano) {
	this.hai_piano = hai_piano;
}
public String getHai_photo() {
	return hai_photo;
}
public void setHai_photo(String hai_photo) {
	this.hai_photo = hai_photo;
}
public String getHai_petition() {
	return hai_petition;
}
public void setHai_petition(String hai_petition) {
	this.hai_petition = hai_petition;
}
public String getHai_video() {
	return hai_video;
}
public void setHai_video(String hai_video) {
	this.hai_video = hai_video;
}
public String getHai_cost() {
	return hai_cost;
}
public void setHai_cost(String hai_cost) {
	this.hai_cost = hai_cost;
}
public String getKeyid() {
	return keyid;
}
public void setKeyid(String keyid) {
	this.keyid = keyid;
}
public String getDokey() {
	return dokey;
}
public void setDokey(String dokey) {
	this.dokey = dokey;
}
public String getNewstempid() {
	return newstempid;
}
public void setNewstempid(String newstempid) {
	this.newstempid = newstempid;
}
public String getClosepl() {
	return closepl;
}
public void setClosepl(String closepl) {
	this.closepl = closepl;
}
public String getInfotags() {
	return infotags;
}
public void setInfotags(String infotags) {
	this.infotags = infotags;
}
public String getIsfava() {
	return isfava;
}
public void setIsfava(String isfava) {
	this.isfava = isfava;
}
public String getResendnum() {
	return resendnum;
}
public void setResendnum(String resendnum) {
	this.resendnum = resendnum;
}
public String getSmalltext() {
	return smalltext;
}
public void setSmalltext(String smalltext) {
	this.smalltext = smalltext;
}
public String getNotimg() {
	return notimg;
}
public void setNotimg(String notimg) {
	this.notimg = notimg;
}
public String getHavefava() {
	return havefava;
}
public void setHavefava(String havefava) {
	this.havefava = havefava;
}
public String getAllphoto() {
	return allphoto;
}
public void setAllphoto(String allphoto) {
	this.allphoto = allphoto;
}
public String getMorepic() {
	return morepic;
}
public void setMorepic(String morepic) {
	this.morepic = morepic;
}
public String getMorepicTotle() {
	return morepicTotle;
}
public void setMorepicTotle(String morepicTotle) {
	this.morepicTotle = morepicTotle;
}
public String getPicsay() {
	return picsay;
}
public void setPicsay(String picsay) {
	this.picsay = picsay;
}
public String getOnlinepath() {
	return onlinepath;
}
public void setOnlinepath(String onlinepath) {
	this.onlinepath = onlinepath;
}
public String getFlashurl() {
	return flashurl;
}
public void setFlashurl(String flashurl) {
	this.flashurl = flashurl;
}
public String getPicurl() {
	return picurl;
}
public void setPicurl(String picurl) {
	this.picurl = picurl;
}
public String getTbname() {
	return tbname;
}
public void setTbname(String tbname) {
	this.tbname = tbname;
}
public String getClassname() {
	return classname;
}
public void setClassname(String classname) {
	this.classname = classname;
}
public String getDownpath() {
	return downpath;
}
public void setDownpath(String downpath) {
	this.downpath = downpath;
}

}
